...
Usage:
    python garmin_pipeline.py <participant_code> [options]
...
from fit_extractor import extract_monitoring_activity
from utils import (
...
def extract_fit_files(fit_zips: list[Path], date_range: tuple = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    stress_df contains columns: stress, body_battery
    """
    import fitparse
...
def render_pdf(daily_df, analysis, fa_df, sessions_df, out_path):
    ...
    with PdfPages(out_path) as pdf:
        ...
        fig2.suptitle("Deep Dive", fontsize=16, fontweight="bold", color=PAL["cyan"], y=0.98)
...
def run(export_dir: Path, out_dir: Path,
    ...
    # 4. Extract minute-level FIT (stress + HR + activity)
    stress_df = hr_df = pd.DataFrame()
    if zips:
        print(f"\n  Parsing FIT files (stress + HR)...")
        stress_df, hr_df = extract_fit_files(zips, date_range=(date_start, date_end))

        print(f"\n  Extracting monitoring activity from FIT files...")
        activity_df = extract_monitoring_activity(zips, date_range=(date_start, date_end))
    else:
    ...
    if not activity_df.empty:
        activity_df.to_csv(out_dir / "garmin_minute_activity.csv")
        print(f"  → garmin_minute_activity.csv")
...
if __name__ == "__main__":
    ...
    # Auto-detect project root: script lives in <root>/scripts/extraction/
