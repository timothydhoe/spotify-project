...
Analyzes and visualizes calm, neutral, and upbeat playlists.
...
Usage:
    ...
    --upbeat data/playlists/bosbes/playlists_generated/bosbes_upbeat_playlist.csv \
...
def print_summary(df_calm, df_upbeat, df_neutral, participant_id):
    ...
    if df_upbeat is not None:
        print(f"  Upbeat: {len(df_upbeat)} songs ({df_upbeat['duration_ms'].sum()/60000:.1f} minutes)")
    if df_neutral is not None:
    ...
    print(f"{'='*80}")
    print(f"{'Feature':<20} {'Calm':<12} {'Upbeat':<12} {'Neutral':<12} {'Difference':<12}")
    print("-" * 80)
    ...
    for feature in features:
        if df_calm is not None and feature in df_calm.columns:
            ...
            upbeat_mean = df_upbeat[feature].mean() if df_upbeat is not None and feature in df_upbeat.columns else np.nan
            neutral_mean = df_neutral[feature].mean() if df_neutral is not None and feature in df_neutral.columns else np.nan
            diff = upbeat_mean - calm_mean if not np.isnan(upbeat_mean) else np.nan
            
            neutral_str = f"{neutral_mean:.2f}" if not np.isnan(neutral_mean) else "N/A"
            diff_str = f"{diff:+.2f}" if not np.isnan(diff) else "N/A"
            
            print(f"{feature:<20} {calm_mean:<12.2f} {upbeat_mean:<12.2f} {neutral_str:<12} {diff_str:<12}")


def validate_iso_adherence(df_calm, df_upbeat, df_neutral, participant_id):
    """
    ...
    playlists = [
        ...
        (df_upbeat, 'upbeat', 'Upbeat (Tired → Energized)'),
...
def create_comparison_plots(df_calm, df_upbeat, df_combined, participant_id):
    ...
    ax.axvspan(80, 130, alpha=0.1, color='#e74c3c', label='Upbeat Target')
...
def validate_playlists(df_calm, df_upbeat, participant_id):
    ...
    upbeat_tempo = df_upbeat['tempo'].mean()
    
    print(f"\n1. TEMPO SEPARATION")
    print(f"   Calm: {calm_tempo:.1f} BPM (target: 50-80)")
    print(f"   Upbeat: {upbeat_tempo:.1f} BPM (target: 80-130)")
    print(f"   Difference: {upbeat_tempo - calm_tempo:.1f} BPM")
    
    if 50 <= calm_tempo <= 80 and 80 <= upbeat_tempo <= 130:
        print("   ✓ PASS")
        checks_passed += 1
    else:
        print("   ✗ FAIL")
    ...
    upbeat_energy = df_upbeat['energy'].mean()
    
    print(f"\n2. ENERGY SEPARATION")
    print(f"   Calm: {calm_energy:.2f} (target: <0.5)")
    print(f"   Upbeat: {upbeat_energy:.2f} (target: >0.6)")
    print(f"   Difference: {upbeat_energy - calm_energy:.2f}")
    
    if calm_energy < 0.5 and upbeat_energy > 0.6:
        print("   ✓ PASS")
        checks_passed += 1
    else:
        print("   ✗ FAIL")
    ...
    print(f"   Upbeat: {upbeat_duration:.1f} min")
...
def generate_report(df_calm, df_upbeat, df_neutral, participant_id, checks_passed, total_checks):
    """Generate text report"""
    
    # Calculate values safely outside the f-string
    calm_count = len(df_calm) if df_calm is not None else 0
    upbeat_count = len(df_upbeat) if df_upbeat is not None else 0
    ...
    upbeat_duration = df_upbeat['duration_ms'].sum()/60000 if df_upbeat is not None else 0
    ...
    upbeat_tempo = df_upbeat['tempo'].mean() if df_upbeat is not None else 0
    ...
    upbeat_energy = df_upbeat['energy'].mean() if df_upbeat is not None else 0
...
Upbeat: {upbeat_count} songs ({upbeat_duration:.1f} min)
Neutral: {neutral_count} songs ({neutral_duration:.1f} min)

KEY METRICS
-----------
                Calm      Upbeat    Neutral
Tempo (BPM)     {calm_tempo:.1f}      {upbeat_tempo:.1f}      {neutral_tempo:.1f}
Energy          {calm_energy:.2f}       {upbeat_energy:.2f}      {neutral_energy:.2f}
...
def main():
    ...
    parser.add_argument('--upbeat', required=True, help='Path to upbeat playlist CSV')
    ...
    # Load data
    print("\nLoading playlists...")
    df_calm, df_upbeat, df_neutral, df_combined = load_playlists(args.calm, args.upbeat, args.neutral)
    
    # Print summary
    print_summary(df_calm, df_upbeat, df_neutral, args.id)
    
    # Validate playlists (traditional checks)
    checks_passed, total_checks = validate_playlists(df_calm, df_upbeat, args.id)
    
    # NEW: Validate ISO adherence
    iso_results = validate_iso_adherence(df_calm, df_upbeat, df_neutral, args.id)
    
    # Create visualizations
    if not args.skip_plots and df_calm is not None and df_upbeat is not None:
        print(f"\nCreating visualizations...")
        create_comparison_plots(df_calm, df_upbeat, df_combined, args.id)
        ...
        if ISO_AVAILABLE:
            ...
            if df_upbeat is not None:
                plot_iso_trajectory(df_upbeat, 'upbeat', args.id, f'{args.id}_iso_upbeat.png')
            if df_neutral is not None:
                plot_iso_trajectory(df_neutral, 'neutral', args.id, f'{args.id}_iso_neutral.png')
            
            # Combined comparison
            plot_combined_iso_comparison(df_calm, df_upbeat, df_neutral, args.id, f'{args.id}_iso_comparison.png')
            
            # Generate ISO report
            save_iso_report(df_calm, df_upbeat, df_neutral, args.id, f'{args.id}_iso_report.txt')
    
    # Generate standard report
    generate_report(df_calm, df_upbeat, df_neutral, args.id, checks_passed, total_checks)
    ...
    if ISO_AVAILABLE:
        ...
        print(f"  - {args.id}_iso_upbeat.png")
        if df_neutral is not None:
